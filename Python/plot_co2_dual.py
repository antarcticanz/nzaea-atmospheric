...
def plot_co2_dual(
    ...
    good = monthly["CO2_ppm"].notna()
    bad  = monthly["CO2_ppm"].isna()
    monthly["filled"] = bad

    if bad.any() and good.sum() >= 2:
        pchip = PchipInterpolator(
            monthly.loc[good, "decdate"].values,
            monthly.loc[good, "CO2_ppm"].values,
        )
        monthly.loc[bad, "CO2_ppm"] = pchip(monthly.loc[bad, "decdate"].values)

    # --------------------------------------------------
    # 4. Growth rate (MATLAB method)
    #    grwthrt(i) = mean(i+1:i+12) - mean(i-12:i-1)
    # --------------------------------------------------
    means = monthly["CO2_ppm"].values
    n = len(means)
    ...
    # print(f"  CO2_ppm range: {monthly.loc[good, 'CO2_ppm'].min():.2f} → {monthly.loc[good, 'CO2_ppm'].max():.2f}")
    # print(monthly[["date", "CO2_ppm", "filled"]].head(10).to_string(index=False))
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers",
        name="Monthly Mean",
        marker=dict(size=6, color="#7ab8d9", opacity=0.95),
        hovertemplate="%{y:.2f} ppm<extra>Monthly Mean</extra>",
        yaxis="y",
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="Annual Change: %{y:.2f} ppm/yr<extra></extra>",
        yaxis="y2",
    ))
